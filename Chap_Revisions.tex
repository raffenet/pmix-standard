...
% SOLT: TODO:  Note that we switch between past and present tense in these 
% sections.  Most recently though we used present tense (4.2)

\begin{compactitemize}
  \item First release prepared using procedures defined in the PMIx
    Governance document
  \item Added specific values to constant definitions to ensure
    consistency across implementations
  \item Added Use-Cases appendix with descriptions for Business Card
    Exchange, Debugging, Hybrid Applications, MPI Sessions, and
    Cross-Version Compatibility
  \item Added guidance on how PMIx defines an Application Binary Interface (ABI)
  \item Added ABI query attributes
  \item Added example C and Python PMIx client codes in sources/ directory
  \item Clarified three roles of consumers of the PMIx interface \#327
  \item Clarify when PMIX_PARENT_ID attribute is provided. % \#376 - dsolt
  \item Clarify value of PMIX_CMD_LINE attribute in spawn case. % \#377 - dsolt
  \item Clarifications to Terms and Conventions chapter and addition of additional term definitions. %  #347 #376 - dsolt
  \item Re-organized the presentation data access, syncronization, reserved keys and non-reserved keys. % \#405 \#175 - dsolt
  \item Made presentation of return values consistent across APIs % \#350 - dsolt
  \item Attributes supported by PRRTE are not longer color coded.  Refer to PRRTE documentation to see what is supported for a particular PRRTE version. % - dsolt
\end{compactitemize}
